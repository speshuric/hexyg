...
- [ ] **1.1. Референсная грамматика (ANTLR/EBNF)**
    - [x] Создать `grammar/hexyg.g4` (не для генерации парсера, а как документация для LLM).
...
- [x] **3.1. Интерфейс командной строки**
    - [x] Подключить `clap`.
    - [x] Реализовать флаги: `--bin-to-hex`, `--hex-to-bin`, `--input`, `--output`.
...
- [ ] **4.2. Функциональность**
    ...
    - [ ] **Compare:** Для IDE возможность сравнивать бинарную часть
...
- [ ] **6.3. Другие (список чтобы не забыть)**
    ...
    - [ ] VS
    - [ ] QT Creator
- [ ] **6.3. Интеграция с [ImHex](https://github.com/WerWolv/ImHex)**
