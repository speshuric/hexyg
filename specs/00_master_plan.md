# Hexator: Сводный план разработки (Master Plan v1.0)
...
* [ ] **0.1. Определение Scope (MVP v0.1)**
    * [ ] Зафиксировать в `README.md`:
        * [ ] Цель: Только Hex ↔ Bin конвертация и валидация.
        * [ ] Приоритет: CLI утилита (Rust).
        * [ ] Редактор: Базовая поддержка VS Code (LSP).
        * [ ] **Non-goals:** Пока без визуального diff, без GUI (только TUI в будущем), без сложных вложенных структур.
* [ ] **0.2. Юридические и организационные вопросы**
    * [ ] Выбрать лицензию (рекомендуется MIT или Apache 2.0).
    * [ ] Создать файл `LICENSE`.
    * [ ] Создать `CONTRIBUTING.md` (правила именования веток, коммитов).
* [ ] **0.3. Настройка Монорепозитория**
    * [ ] Инициализировать git.
    * [ ] Настроить `.gitignore` (комбинация Rust, Node, Kotlin, Python, C#).
...
* [ ] **1.1. Референсная грамматика (ANTLR/EBNF)**
    * [ ] Создать `grammar/Hexator.g4` (не для генерации парсера, а как документация для LLM).
    * [ ] Описать токены: `ADDRESS`, `HEX_BYTE`, `COMMENT`, `DIRECTIVE` (`#option`), `ASCII_PREVIEW`.
...
* [ ] **2.1. Базовая архитектура (Crate: `hexator-core`)**
...
* [ ] **3.2. I/O Pipeline**
    * [ ] Поддержка `stdin` / `stdout` для работы в пайпах (`echo "ABC" | hexator`).
