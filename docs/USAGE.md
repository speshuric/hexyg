...
- `--to={addr}` &ndash; end address can be decimal (such as 987) or hexadecimal with `0x` prefix (such as 0x4FE). Negative decimal works as the end-based address (as in slices in Python, for example). Addresses outside the file boundaries cause an error (non-zero return code). Default is the last address in the file.
...
- `--address-size-step={N}` for arbitrary length of address set the minimal step of number of digits in address. Default is 1. Set `address_size_step` option in file.
- `--line-length={N}` line length in bytes. Default is 16. Set `line_length` option in file.
- `--block-length={N}` when given line is divided to blocks of this length. Default is 0.  
- `--byte-separator=""` empty or whitepace string to separate bytes
- `--block-separator=" "` empty or whitepace string to separate blocks of bytes in one line
